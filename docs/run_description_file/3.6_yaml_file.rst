...
Example for Runs on Digital Research Alliance of Canada Systems
---------------------------------------------------------------

This is an example run description file for use on :kbd:`nibi`.
...
:kbd:`account`
  *Optional.*
  ...
  This key-value pair is required on systems like :kbd:`nibi.alliancecan.ca` that use the `Slurm Workload Manager`_ and that you submit runs to with the :command:`sbatch` command.
...
:kbd:`runs directory`
  The path to the directory where run directories will be created by the :command:`pixi run nemo prepare`) sub-command.
...
The :command:`pixi run nemo prepare` sub-command and the :py:func:`nemo_cmd.api.prepare` API function confirm that the targets of the symlinks exist,
...
The :command:`pixi run nemo run` and :command:`pixi run nemo prepare` commands and the :py:func:`nemo_cmd.api.prepare` API function confirm that the targets of the symlinks exist,
and exit with an error message if not.
...
:kbd:`separate XIOS server`
  ...
  The :command:`pixi run nemo prepare` sub-command sets the value of the :kbd:`using_server` variable in the :kbd:`xios` context in the copy of the :file:`iodef.xml` file in the temporary run directory to reflect the :kbd:`separate XIOS server` value.

:kbd:`XIOS servers`
  The number of XIOS servers to run when the value of :kbd:`separate XIOS server` it :py:obj:`True`.
  The number of XIOS servers is added to the number of NEMO processors calculated from the :kbd:`MPI decomposition` value to specify the total number of processors requested in the :kbd:`#PBS` directives section of the :file:`NEMO.sh` script generated by the :command:`pixi run nemo run` sub-command.
...
the :command:`pixi run nemo prepare` command will generate a warning message like::
...
.. note::
    The :command:`pixi run nemo` :ref:`nemo-run` uses information in the run description files to provide the  values for the following resources for you:
...
:command:`pixi run nemo run` will calculate :kbd:`n` for you based on the values you give for :kbd:`p`,
:kbd:`MPI decomposition`,
...
* the nco library to make the :command:`ncks` available for the :command:`pixi run nemo` :ref:`nemo-deflate`
